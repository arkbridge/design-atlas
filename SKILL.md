...
- Reference grid where each card carries:
  ...
  - **Feedback row** (NEW):
    ...
    but NOT: ________________________________________ (optional one-line)
    ```
...
- On any input change, walk the DOM, build a JSON object: `{global_register_steer, surfaces: {surface_id: {approach, screens: {screen_id: {direction, steer, refs: {ref_idx: {rating, note, but_not}}}}}}}`
...
1. Parse it
2. Confirm interpretation back to the user in a short summary: per-surface counts of hero/use-this/anti-pattern + the steers verbatim
...
1. **Locate candidate variants** across `hero` + `use this` rated references. Aim for 3-6 distinct visual variants per category.
...
- `use this` refs weight 2
- `ok` refs weight 1
- `skip`/`anti-pattern` refs weight 0 (or negative for anti-pattern)
